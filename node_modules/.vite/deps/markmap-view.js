...
function get(type2, name2) {
  for (var i = 0, n = type2.length, c3; i < n; ++i) {
    if ((c3 = type2[i]).name === name2) {
      return c3.value;
    }
  }
}
function set2(type2, name2, callback) {
...
Path.prototype = path.prototype = {
  ...
  arc: function(x3, y3, r, a0, a1, ccw) {
    ...
    var dx = r * Math.cos(a0), dy = r * Math.sin(a0), x02 = x3 + dx, y0 = y3 + dy, cw = 1 ^ ccw, da = ccw ? a0 - a1 : a1 - a0;
    if (r < 0) throw new Error("negative radius: " + r);
    if (this._x1 === null) {
      this._ += "M" + x02 + "," + y0;
...
function polygonContains_default(polygon, point6) {
  ...
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (var j = 0; j < m; ++j, lambda0 = lambda1, sinPhi0 = sinPhi1, cosPhi0 = cosPhi1, point0 = point1) {
      ...
      sum3.add(atan2(k2 * sign3 * sin2(absDelta), cosPhi0 * cosPhi1 + k2 * cos2(absDelta)));
  ...
  return (angle < -epsilon4 || angle < epsilon4 && sum3 < -epsilon22) ^ winding & 1;
}
...
function azimuthalInvert(angle) {
  return function(x3, y3) {
    var z = sqrt(x3 * x3 + y3 * y3), c3 = angle(z), sc = sin2(c3), cc = cos2(c3);
    return [
      atan2(x3 * sc, z * cc),
      asin(z && y3 * sc / z)
    ];
...
var gamma_default = function sourceRandomGamma(source) {
  ...
  function randomGamma(k2, theta) {
    ...
    return function() {
      do {
        do {
          var x3 = randomNormal(), v = 1 + c3 * x3;
        } while (v <= 0);
        v *= v * v;
        var u = 1 - source();
      } while (u >= 1 - 0.0331 * x3 * x3 * x3 * x3 && Math.log(u) >= 0.5 * x3 * x3 + d * (1 - v + Math.log(v)));
...
function link2(curve) {
  ...
  link3.x = function(_) {
    return arguments.length ? (x3 = typeof _ === "function" ? _ : constant_default11(+_), link3) : x3;
  };
  link3.y = function(_) {
    return arguments.length ? (y3 = typeof _ === "function" ? _ : constant_default11(+_), link3) : y3;
  };
...
function curveHorizontal(context, x02, y0, x12, y1) {
  context.moveTo(x02, y0);
  context.bezierCurveTo(x02 = (x02 + x12) / 2, y0, x02, y1, x12, y1);
}
...
function zoom_default2() {
  ...
  function mousedowned(event, ...args) {
    ...
    var g = gesture(this, args, true).event(event), v = select_default2(event.view).on("mousemove.zoom", mousemoved, true).on("mouseup.zoom", mouseupped, true), p = pointer_default(event, currentTarget), currentTarget = event.currentTarget, x02 = event.clientX, y0 = event.clientY;
...
// node_modules/markmap-view/dist/index.esm.js
var Hook = class {
  constructor() {
    this.listeners = [];
  }
...
function childSelector(filter3) {
  if (typeof filter3 === "string") {
    const tagName = filter3;
    filter3 = (el) => el.tagName === tagName;
  }
  const filterFn = filter3;
...
Node$1.prototype = hierarchy2.prototype = {
  constructor: Node$1,
  count: node_count,
  each: node_each,
...
function flextree(options) {
  ...
  function getFlexNode() {
    ...
    return class FlexNode extends hierarchy2.prototype.constructor {
      ...
      copy() {
        const c3 = wrap(this.constructor, this, (node) => node.children);
        c3.each((node) => node.data = node.data.data);
        return c3;
      }
      get size() {
  ...
  Object.assign(layout, {
    ...
    dump(tree) {
      ...
      const _dump = (i0) => (node) => {
        ...
        return `{ size: [${size.join(", ")}],${i1}x: ${x3}, y: ${y3}${kdumps}},`;
      };
      return _dump("\n")(tree);
    }
  });
...
flextree.version = version2;
var layoutChildren = (w, y3 = 0) => {
  w.y = y3;
...
var resolveX = (w, prevSum, parentX) => {
  ...
  const sum3 = prevSum + w.relX;
  w.relX = sum3 + w.prelim - parentX;
  w.prelim = 0;
  w.x = parentX + w.relX;
  (w.children || []).forEach((k2) => resolveX(k2, sum3, w.x));
  return w;
};
var shiftChange = (w) => {
...
var updateLows = (lowY, index2, lastLows) => {
...
var isLeaf = function isLeaf2(c3) {
  return typeof c3 === "string" || typeof c3 === "number";
};
var isElement = function isElement2(c3) {
  return (c3 == null ? void 0 : c3.vtype) === VTYPE_ELEMENT;
};
var isRenderFunction = function isRenderFunction2(c3) {
  return (c3 == null ? void 0 : c3.vtype) === VTYPE_FUNCTION;
};
...
function setDOMAttribute(el, attr, value, isSVG) {
  ...
  if (value === true) {
    ...
    var namespace = isSVG ? NS_ATTRS[attr] : void 0;
    if (namespace !== void 0) {
...
function mount(vnode, env) {
  ...
  if (isRenderFunction(vnode)) {
    var _ref = vnode, type2 = _ref.type, props = _ref.props;
    if (type2 === Fragment) {
...
function flattenWithoutNull(array3) {
  var result = [];
...
function minBy(numbers2, by) {
  const index2 = minIndex(numbers2, by);
  return numbers2[index2];
}
...
var defaultColorFn = ordinal(category10_default);
var isMacintosh = typeof navigator !== "undefined" && navigator.userAgent.includes("Macintosh");
var Markmap = class _Markmap {
  constructor(svg2, opts) {
    this.revokers = [];
    ["handleZoom", "handleClick", "handlePan"].forEach((key) => {
      this[key] = this[key].bind(this);
    });
    this.viewHooks = createViewHooks();
    this.svg = svg2.datum ? svg2 : select_default2(svg2);
    this.styleNode = this.svg.append("style");
    this.zoom = zoom_default2().filter((event) => {
  ...
  handlePan(e) {
    ...
    const newTransform = transform2.translate(-e.deltaX / transform2.k, -e.deltaY / transform2.k);
  ...
  initializeData(node) {
    ...
    const nodes = arrayFrom(container.childNodes).map((group2) => group2.firstChild);
  ...
  setOptions(opts) {
    this.options = _extends({}, _Markmap.defaultOptions, opts);
  }
  setData(data, opts) {
  ...
  renderData(originData) {
    ...
    const layout = flextree().children((d) => {
      ...
      return a2.parent === b.parent ? spacingVertical : spacingVertical * 2;
    });
    ...
    const x02 = (_origin$data$state$x = origin.data.state.x0) != null ? _origin$data$state$x : origin.x;
    ...
    const nodeEnter = node.enter().append("g").attr("data-depth", (d) => d.data.depth).attr("data-path", (d) => d.data.state.path).attr("transform", (d) => `translate(${y0 + origin.ySizeInner - d.ySizeInner},${x02 + origin.xSize / 2 - d.xSize})`);
    ...
    const circle2 = nodeMerge.selectAll(childSelector("circle")).data((d) => d.data.children ? [d] : [], (d) => d.data.state.key).join((enter) => {
    ...
    this.transition(circle2).attr("r", 6).attr("cx", (d) => d.ySizeInner).attr("cy", (d) => d.xSize).attr("stroke", (d) => color2(d.data)).attr("fill", (d) => {
    ...
    const path2 = this.g.selectAll(childSelector("path")).data(links, (d) => d.target.data.state.key).join((enter) => {
      const source = [y0 + origin.ySizeInner, x02 + origin.xSize / 2];
  ...
  async fit() {
    ...
    const initialZoom = identity4.translate((offsetWidth - naturalWidth * scale) / 2 - minY * scale, (offsetHeight - naturalHeight * scale) / 2 - minX * scale).scale(scale);
    return this.transition(this.svg).call(this.zoom.transform, initialZoom).end().catch(noop3);
  }
  /**
  ...
  async ensureView(node, padding) {
    ...
    const transform2 = transform(svgNode2);
    const [left, right] = [itemData.y, itemData.y + itemData.ySizeInner + 2].map((x3) => x3 * transform2.k + transform2.x);
    const [top, bottom] = [itemData.x - itemData.xSize / 2, itemData.x + itemData.xSize / 2].map((y3) => y3 * transform2.k + transform2.y);
    const pd = _extends({
      left: 0,
      right: 0,
      top: 0,
    ...
    const dx = dxs[0] * dxs[1] > 0 ? minBy(dxs, Math.abs) / transform2.k : 0;
    const dy = dys[0] * dys[1] > 0 ? minBy(dys, Math.abs) / transform2.k : 0;
    if (dx || dy) {
      const newTransform = transform2.translate(dx, dy);
      return this.transition(this.svg).call(this.zoom.transform, newTransform).end().catch(noop3);
    }
  }
  /**
  ...
  async rescale(scale) {
    ...
    const transform2 = transform(svgNode2);
    const newTransform = transform2.translate((halfWidth - transform2.x) * (1 - scale) / transform2.k, (halfHeight - transform2.y) * (1 - scale) / transform2.k).scale(scale);
    return this.transition(this.svg).call(this.zoom.transform, newTransform).end().catch(noop3);
  }
  destroy() {
  ...
  static create(svg2, opts, data) {
    const mm = new _Markmap(svg2, opts);
    if (data) {
      mm.setData(data);
      mm.fit();
    }
...
function deriveOptions(jsonOptions = {}) {
  ...
  } else if (color2 != null && color2.length && typeof color2[0] === "string") {
    const colorFn = ordinal(color2);
    opts = _extends({}, opts, {
  ...
  numberKeys.forEach((key) => {
    ...
    if (typeof value === "number") opts = _extends({}, opts, {
      [key]: value
    });
  });
  return opts;
...
markmap-view/dist/index.esm.js:
  (*! markmap-view v0.13.4 | MIT License *)
